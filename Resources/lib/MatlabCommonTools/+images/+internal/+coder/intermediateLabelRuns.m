...
coder.internal.errorIf(~eml_is_const(mode), ...
    'images:validate:codegenInputNotConst','mode');
...
for col = 1:N
    offset = coder.internal.indexInt((col-1)*M);
    
    row = coder.internal.indexInt(1);
    while (row <= M)
        % Look for the next run.
        while ((row <= M) && (im(row+offset) == 0))
            row = row + 1;
        end
        
        if ((row <= M) && (im(row+offset) ~= 0))
            c(runCounter) = col;
            sr(runCounter) = row;
            while ((row <= M) && (im(row+offset) ~= 0))
                row = row + 1;
