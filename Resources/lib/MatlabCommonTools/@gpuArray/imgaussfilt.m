...
if separableFlag
    ...
    if useConvPadding
        ...
        if ~strcmp(classUnderlying(A),dtype)
            A = cast(A,dtype);
        end
    ...
    % For filtering an image-stack, we must loop over each slice.
    B = gpuArray.zeros(outSize, dtype);
    subSlice.type = '()';
    subSlice.subs = {':',':',1};
    for dim = 1 : prod(outSize(3:end))
        
        subSlice.subs{3} = dim;
        %B(:,:,dim) = cast(conv2(hRow, hCol, A(:,:,dim), 'valid'), dtype);        
        B = subsasgn(B, subSlice,...
            cast(conv2(hCol, hRow, subsref(A, subSlice), 'valid'), dtype));
    end
    A = B;
...
minKernelElems = getSeparableFilterThreshold(classUnderlying(A));
...
if ismatrix(A)
    ...
    for dim = 1 : dims3toEnd
        ...
        A = subsasgn(A, subSlice, ifft2( fft2(subsref(A,subSlice), fftSize(1), fftSize(2)) .* fftH, 'symmetric' ) );
    end
end

% cast back to input type
if ~strcmp(dtype,classUnderlying(A))
    A = cast(A, dtype);
end
...
hValidateAttributes(A, supportedClasses, supportedImageAttributes, mfilename, 'A');
