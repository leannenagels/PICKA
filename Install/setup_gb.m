...
% Folders to test in that order
google_drive_folders = {};
if ismac()
    google_drive_folders{end+1} = '/Volumes/GoogleDrive'; % For Google Drive File Stream
end
google_drive_folders{end+1} = fullfile(getHome(), 'Google Drive');
google_drive_folders{end+1} = fullfile(getHome());

% Testing if any of these folders exist
google_drive_folder = '.';
for i=1:length(google_drive_folders)
    if exist(google_drive_folders{i}, 'dir')
        google_drive_folder = google_drive_folders{i};
        break
    end
end

src = uigetdir(google_drive_folder, 'PICKA installation: Source directory (Google Drive)');
