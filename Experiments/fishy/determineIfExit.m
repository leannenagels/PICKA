function [results, expe, terminate, nturns] = determineIfExit(results, expe, steps, ...
    ...
    if nturns >= options.(options_phase).terminate_on_nturns 
...
%         i_tp = i_nz(i_d)+1;
        ...
        i_tp = i_tp(end-(options.(options_phase).threshold_on_last_n_trials-1):end);
    ...
    elseif length(response_accuracy) >= options.(options_phase).terminate_on_ntrials
        ...
        if expe.( phase ).conditions(i_condition).attempts >= options.(options_phase).retry + 1
        ...
        terminate = true;
    elseif length(response_accuracy) >= options.(options_phase).change_step_size_n_trials ...
            && sum(response_accuracy(end - (options.(options_phase).change_step_size_n_trials-1) : end) == 0) >= ...
            (options.(options_phase).change_step_size_n_trials / 4 * 3) % PT if at least half of the responses are wrong stop
            
        %    && all(response_accuracy(end - (options.(options_phase).change_step_size_n_trials-1) : end) == 0)
