function results2txt
    ...
    cd('C:/Users/leann/Pronouns/ObjPro');

    fileID = fopen('stimuliSetup_new.txt', 'wt');
%      fprintf(fileID,'subID\tphase\tvoice\tface\tcongruency\tacc\tRT \n');
    fprintf(fileID,'item\image_pronoun\image_reflexive\soundFile_pronoun\soundFile_reflexive\n'); %\tcongruency

    fileAve = fopen('stimuliSetup_new2.txt', 'wt');
    fprintf(fileAve,'item\image_pronoun\image_reflexive\soundFile_pronoun\soundFile_reflexive\n'); %\tcongruency
...
function exportDataOldExtended(ppID, resp, fileID, fileAve, phase, cue, attempt)
        ...
        for iresp = 1 : nResponses
            ...
            fprintf(fileID,'%s\t', resp(iresp).button_clicked);
