function [G, TVScreen, Buttonup, Buttondown, Speaker, gameCommands, Hands] = gender_game(options)
    ...
    % EG: TODO Automatically rescale background (see Fishy)
    ...
    SpriteKit.Background(resizeBackgroundToScreenSize(screen2, fullfile(options.locationImages, 'genderbackground3_unscaled.png')));
    addBorders(G);
    
    TVScreen = SpriteKit.Sprite('tvscreen');
    TVScreen.initState('off', ones(1,1,3),true); % whole screen green
    TVScreen.initState ('noise', [options.locationImages 'TVScreen_noise.png'], true);
    for iwoman=1:7
...
%     TVScreen.Location = [screen2(3)/2.42, screen2(4)/2.02]; % Debi's
    ...
    Buttonup.initState ('on',[options.locationImages 'button_right.png'], true);
    Buttonup.initState('press', [options.locationImages 'button_right_pressed.png'], true)
    Buttonup.initState ('off', ones(1,1,3), true);
    Buttonup.Location = [screen2(3)/1.65, screen2(4)/5.5];
    Buttonup.State = 'off';
    [HeightButtonup, WidthButtonup] = size(imread ([options.locationImages 'button_right.png']));
    ...
    Buttondown.initState ('on',[options.locationImages 'button_wrong.png'], true);
    Buttondown.initState ('press', [options.locationImages 'button_wrong_pressed.png'], true);
    Buttondown.initState ('off', ones(1,1,3), true);
    Buttondown.Location = [screen2(3)/1.40, screen2(4)/5.5];
    Buttondown.State = 'off';
    [HeightButtondown, WidthButtondown] = size(imread ([options.locationImages 'button_wrong.png']));
